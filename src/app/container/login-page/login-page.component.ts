...
export class LoginPageComponent /*implements OnInit*/{
  ...
  Login(form: NgForm) {
    ...
    this.userData = new Login(form.value.username, form.value.password);

    console.log(this.userData);

    this.http.post('http://localhost:8080/login', this.userData).subscribe((response)=>{console.log(response)});
  }
